...
const PageWrap = ({ children }) => (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-8">
        ...
        <div className="w-full max-w-sm">
            <div className="bg-white rounded-3xl shadow-2xl p-7 border border-slate-100 space-y-5">
                {children}
            </div>
        </div>
    </div>
    ...
    const Btn = ({ children, onClick, disabled, color = 'emerald', type = 'submit' }) => {
        ...
        return (
            <button type={type} onClick={onClick} disabled={disabled || loading}
                className={`w-full ${colors[color]} disabled:opacity-50 disabled:cursor-not-allowed text-white py-3.5 rounded-xl font-black text-sm flex items-center justify-center gap-2 transition-all shadow-lg active:scale-[0.98]`}>
        ...
        <PageWrap>
            ...
            <div className="text-center space-y-1">
                <div className="w-14 h-14 bg-emerald-50 rounded-2xl flex items-center justify-center mx-auto">
                    {isLogin ? <LogIn size={24} className="text-emerald-600" /> : <UserPlus size={24} className="text-emerald-600" />}
                </div>
            ...
            <button type="button" onClick={handleGoogle}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-white border border-slate-200 hover:border-emerald-200 hover:bg-emerald-50 transition-all font-bold text-slate-700 shadow-sm text-sm">
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                            ...
                            <input type="text" required value={name} onChange={e => setName(e.target.value)}
                                ...
                                className="w-full bg-slate-50 border border-slate-200 focus:border-emerald-400 focus:bg-white rounded-xl py-3 pl-10 pr-4 outline-none transition-all font-bold text-slate-900 text-sm" />
                        </div>
                    </div>
                )}
            ...
            <div className="text-center">
                <button type="button" onClick={() => { setMode(isLogin ? 'register' : 'login'); setBanner(null); }}
                    className="text-[10px] font-black text-slate-400 hover:text-emerald-600 transition-colors uppercase tracking-wide">
